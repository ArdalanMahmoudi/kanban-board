...
const List = ({ list }: { list: CardList }) => {
  ...
  return (
    <div className="h-full flex flex-col gap-4 border rounded-sm">
      {/* Title */}
      <div className={`w-full flex flex-col rounded-sm ${colors.header}`}>
        <div className="flex justify-between items-center p-4 ">
          <div className="flex items-center gap-1">
            <h3 className="text-lg font-heading ">{list.status}</h3>
            {/* Badge */}
            <Badge className={`${colors.badge}`}>{list.cards.length}</Badge>
          </div>
          {/* Add-Task */}
          <AddCardButton
            listId={list.id}
            DialogTriggerButton={
              <Button variant={"ghost"} className={`${colors.header}`}>
        ...
        <AnimatePresence>
          {list.cards.length === 0 && (
            <motion.div
              ...
              className="absolute inset-0 w-full flex justify-center items-center flex-col gap-4 p-4 text-sm text-gray-500"
            >
